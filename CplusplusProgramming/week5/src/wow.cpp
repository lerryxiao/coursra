...
Dragon::Dragon(int number, int strength, std::string name, int headStrength)
        : Warrior(number, strength, name), headStrength(headStrength)
{
    setWeapon();
...
Lion::Lion(int number, int strength, std::string name, int headStrength)
        : Warrior(number, strength, name), headStrength(headStrength)
{
...
Wolf::Wolf(int number, int strength, std::string name)
        : Warrior(number, strength, name)
{}
...
Headquarter::Headquarter(std::string name,
                         int strength,
                         int dragonStrength,
                         int ninjaStrength,
                         int icemanStrength,
...
void RedHeadquarter::makeWrrior()
{
...
void BlueHeadquarter::makeWrrior()
{
    //lion、dragon、ninja、iceman、wolf
...
int main()
{
    int count = 0;
    int allHP = 0;
    int dragon, ninja, iceman, lion, wolf = 0;
    std::cin >> count;
    int clock = 0;
    bool canMake;
    for (int i = 0; i < count; i++)
    {
        ...
        Headquarter *redHead = &red;
        Headquarter *blueHead = &blue;
        while (canMake)
        {
            if (!redHead->hasStop)
            {
            ...
            if (redHead->hasStop && blueHead->hasStop)
            {
                canMake = false;
            }
        }
